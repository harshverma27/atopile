...
def _give_pad(g, tg, electrical: F.Electrical) -> None:
    """Make an electrical eligible for net binding by giving it a lead and pad."""
...
def test_power_rail_members_default_net_names():
    """Without an override, ElectricPower members keep their suggested names."""
    g, tg = _make_graph()
...
def test_override_net_name_on_power_rail_members():
    """
    override_net_name must win on ElectricPower.hv/.lv.

    ElectricPower pre-attaches a SUGGESTED has_net_name_suggestion ("hv"/"lv")
    to each member. An override adds a second, EXPECTED instance to the same
    node. Reading only the first instance returns the SUGGESTED one and drops
    the override.
    """
    g, tg = _make_graph()
...
def test_override_net_name_on_standalone_electrical():
    """The documented workaround keeps working: override on a plain Electrical."""
    g, tg = _make_graph()
